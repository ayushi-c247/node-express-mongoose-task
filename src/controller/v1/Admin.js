...
const login = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: message.ADMINLOGIN_ERROR });
    }
}
...
const userStatusUpdate = async (req, res) => {
    try {
        ...
        console.log("userStatusUpdate", error);
        return res.status(500).json({ error: message.USERSTATUSUPDATE_ERROR });
    }
}
...
const deleteUser = async (req, res) => {
    try {
        const id = req.params.id;
        const blog = await blogModel.findOne({ authorId: id });
        ...
        return res.status(500).json({ error: message.DELETEUSER_ERROR });
    }
}
...
const display = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: message.DISPLAY_ERROR });
    }
}
...
const adminDashboard = async (req, res) => {
    try {
        ...
        console.log("adminDashboard", error);
        return res.status(500).json({ error: message.ADMINDASHBOARD_ERROR });
    }
}
...
const logout = async (req, res) => {
    try {
        if (req.session.user && req.cookies.user_sid) {
            ...
            res.redirect(`${process.env.hostPath}v1/admin/login`);
        } else {
            res.redirect(`${process.env.hostPath}v1/admin/login`);
        }
    } catch (error) {
        console.log("logout", error);
        return res.status(500).json({ error: message.LOGOUT_ERROR });
    }
}

module.exports = { adminLogin, homePage, login, deleteUser, display, userStatusUpdate, adminDashboard, logout }
